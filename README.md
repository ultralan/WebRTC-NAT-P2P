...
cd device
go mod download
go run main.go -id device-1 -server ws://localhost:8080/ws
```

参数说明：
- `-id`: 设备ID（默认: device-1）
- `-server`: 信令服务器地址（默认: ws://localhost:8080/ws）
...
3. Device连接时使用公网地址：
```bash
go run main.go -server ws://your-server-ip:8080/ws
```

4. Frontend修改WebSocket地址（在 `src/App.jsx` 中）：
```javascript
const ws = new WebSocket(`ws://your-server-ip:8080/ws?id=${clientId.current}&type=frontend`)
```
...
连接: `ws://localhost:8080/ws?id=<client-id>&type=<device|frontend>`
